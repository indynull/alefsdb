...
| `getfattr -n user.alefs.member` | Display hint for set members (hash-named dirents) |
...
| set | Hash-named member files + `user.alefs.member` xattr | Unlink removes member |
...
- Full POSIX rename/reorder of list indices via arbitrary editor temp files (`rename` → `ENOTSUP`)
